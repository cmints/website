...
site.queryPages | {helper-type-func} | <a href="#site.querypages">{helper-desc-query-pages[Helper description] Query pages using page metadata.</a>}
...
{site-queryPages-p1[Paragraph in 'site.queryPages' section]
The metadata is an array of pages Front Matter metadata with additional <fix>`path`</fix> property, which holds the pathname or the permalink if one exist. <fix>`site.queryPages`</fix> is using <fix><a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/filter" target="_blank">Array.prototype.filter()</a></fix> to filter the metadata.
}
...
<% docPages.forEach(function(item) { %>
  <a <%- i18n.href(item.path) %>> <%= item.title %> </a>
<% }); %>
```
